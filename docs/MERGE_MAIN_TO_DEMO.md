...
10. **resolve_embedding_config 不读 DB 的 model** line 52 硬编码 `settings.EMBEDDING_MODEL`，忽略 `sys_cfg.embedding_model`。导致页面配的 Embedding 模型名完全无效。合并后检查该函数是否读了 `sys_cfg.embedding_model`
